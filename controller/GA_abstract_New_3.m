function [plan,plan_state,plan_cost,num_spoint] = GA_abstract_New_3(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks)
abstracted = true;
...
%             num_sparent(num_rep)=num_sparent(num_rep)+1;
    ...
    for i=1:length(mutation_idx)
        ...
        rng('shuffle');
        idx_mut=randi([1 ceil((length(mut_off)-1)/2)]);
