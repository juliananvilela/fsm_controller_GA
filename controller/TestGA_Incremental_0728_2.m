...
while num_rep<=100 && aux_same_result<=20
    ...
    for i=1:length(parents)/2
        ...
        if ~isempty(cross_point)
            ...
            [offspring{2*i-1},offspring_event{2*i-1},aux_dup]=remove_cycles(offspring_aux,offspring_event_aux);
            ju=ju+aux_dup;
            offspring_aux=[];
            offspring_event_aux=[];
            ...
            [offspring{2*i},offspring_event{2*i},aux_dup]=remove_cycles(offspring_aux,offspring_event_aux);
            ju=ju+aux_dup;
