function [plan,plan_state,plan_cost] = GA_abstract(population_size,max_rep,mutation_rate,stop_cond,states,start_loc,numTasks,numRobots,numRegions,tasks,regions,robot_tasks)
abstracted = true;
...
for i=1:length(path_gen)
        ...
        path_gen{i}='';
        path_gen{i}=check_loops_fix_new(old_path_gen); %remove the loops
end
    ...
    for j=1:length(path_gen{i})-1
        ...
        for m=1:sz(1,1)
            ...
            if aux_1==aux_2
                ...
                event_bot = states{act_state,2}{m,1}(1)-96;
                ...
                calcTransitionCost(states{act_state,2}{m,1},act_state,aux_2,path_gen{i}(1:j),states,tasks,0,regions,numRegions,start_loc,...
                ...
                for jj = 1:numRobots
                    robotLocation{str2double(aux_2),jj} = robotLocation{act_state,jj};
...
while num_rep<=max_rep && aux_same_result<=stop_cond
    ...
    for i=1:length(parents)/2
        ...
        if ~isempty(cross_point)
            ...
            offspring{2*i}=[parent_2(1:cross_point{idx}(2)-1),parent_1(cross_point{idx}(1):end)];
    ...
    for i=1:length(offspring)
        ...
        offspring{i}='';
        offspring{i}=check_loops_fix_new(old_offspring);
    end
        ...
        for j=1:length(offspring{i})-1
            ...
            for m=1:sz(1,1)
                ...
                if aux_1==aux_2
                    ...
                    %path_gen_old{max_path+i}
                    [transition_cost,~,new_X,new_Y] = ...
                        calcTransitionCost(states{act_state,2}{m,1},act_state,str2double(aux_2),offspring{i}(1:j),states,tasks,0,regions,numRegions,start_loc,...
                        numTasks,abstracted,start_events_only,robotLocation{act_state,event_bot},robot_tasks,0);
